 ...
 */
export class SemanticSegmentationModule extends BaseLabeledModule {
   ...
   */
  async forward(imageSource, classesOfInterest = [], resizeToInput = true) {
    if (this.nativeModule == null) {
      throw new RnExecutorchError(RnExecutorchErrorCode.ModuleNotLoaded, 'The model is currently not loaded.');
    }
    const classesOfInterestNames = classesOfInterest.map(label => String(label));
    const nativeResult = await this.nativeModule.generate(imageSource, classesOfInterestNames, resizeToInput);
    return nativeResult;
  }
}
